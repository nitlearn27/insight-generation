...
- Providers, all OpenAI-/Gemini-/Anthropic-shaped: `_call_nvidia`, `_call_gemini`, `_call_glm`, `_call_ainative`, `_call_openrouter`. Each is enabled only if its API key is set; OpenRouter is always present and kept **last** as the shared-cap fallback. `_call_nvidia` enforces `response_format: {"type":"json_object"}` (NVIDIA NIM's llama-3.1-8b accepts JSON mode; without it the small model returns prose). `_call_llm` also validates that each provider's body is JSON-parseable and falls through to the next provider if not.
